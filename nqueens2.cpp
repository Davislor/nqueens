 ...
 * a SIMD instruction such as PAND.  This is probably not the absolutely most
 * compact representation (i.e., the 5x5 board will probably need 5 bytes
...
// We should not need to override the other implicit member functions.
...
template <unsigned int N>
  board<N>::board (void) : squares( (size_t)N )
  {
    return;
  }
...
// Wipes all queens off the board.
{
  squares = vector< bitset<N> >( (size_t)N );
}
